...
export default function RemedyScorer({ initialQuery = "", initialCategory = "", category }: RemedyScorerProps) {
  ...
  // Fetch health profile on mount so AI can personalize results
  useEffect(() => {
    getHealthProfile()
      .then((profile) => { if (profile) setHealthProfile(profile); })
  ...
  const submitToAI = (syms: string[], answers: Record<string, string>) => {
    ...
    // Pass health profile to backend so Groq can personalize
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        setSymptoms((prev) => prev.filter((s) => s !== placeholder).concat(`Report: ${file.name}`));
        setAnalyzingFiles((prev) => prev.filter((n) => n !== file.name));
      }
    }
    e.target.value = "";
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                {/* Personalization badge — shown when health profile is available */}
            ...
            <div className="space-y-4">
              {topResults.map((result: any, idx: number) => (
                <div key={result.remedy.id} className="border rounded-xl p-4 bg-white">
                  ...
                  {/* Groq AI insight — personalized explanation */}
                  {result.ai_insight && (
            ...
            <div>
              ...
              {healthProfile && (healthProfile.name || healthProfile.age) && (
                <p className="text-xs text-blue-600 mt-1">
                  👤 Using profile: {healthProfile.name || ""}
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 p-3 bg-neutral-50 rounded-lg">
                      {symptoms.map((s) => (
                        <Badge
                          ...
                          onClick={() => { if (!s.startsWith("🔍 Analyzing:")) handleRemoveSymptom(s); }}
                        >
