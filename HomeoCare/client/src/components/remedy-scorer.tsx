...
import { Search, TrendingUp, ArrowLeft, HelpCircle, Camera, Paperclip, X, CheckCircle } from "lucide-react";
...
export default function RemedyScorer({ initialQuery = "", initialCategory = "", category }: RemedyScorerProps) {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach(f => {
      setSymptoms(prev => [...prev, `Clinical report: ${f.name}`]);
    });
  };
      ...
      <div>
        ...
        <div className="max-w-2xl mx-auto px-4">
          <Card className="p-6">
            ...
            {/* Also allow typing custom symptom during Q */}
  ...
  if (showResults && scoreMutation.data) {
    const topResults = scoreMutation.data.slice(0, 3); // Only top 3
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 p-3 bg-neutral-50 rounded-lg">
                      {symptoms.map(s => (
                        <Badge key={s} variant="secondary" className="cursor-pointer hover:bg-red-100"
                          onClick={() => handleRemoveSymptom(s)}>{s} ×</Badge>
                      ))}
                    </div>
                  </div>
                )}
