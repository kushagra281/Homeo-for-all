...
import { Search, LogOut, User, Clock, ChevronRight, Paperclip, ArrowLeft } from "lucide-react"
...
// Declare google translate on window
declare global {
  interface Window {
  ...
  useEffect(() => {
    ...
    window.googleTranslateElementInit = () => {
      new window.google.translate.TranslateElement(
        {
          pageLanguage: "en",
          includedLanguages: "hi,bn,te,mr,ta,gu,kn,ml,pa,ur,en",
              ...
              {history.length === 0 ? (
                ...
                <div className="space-y-3">
                  {history.slice(0, 10).map((h: any, i: number) => (
                    <div key={i}
                      className="p-3 bg-green-50 rounded-lg border border-green-100 cursor-pointer hover:border-green-300 transition"
                      onClick={() => {
                        setShowProfile(false)
                        setLocation(`/symptom-analysis?q=${encodeURIComponent(h.symptoms?.join(", ") || "")}`)
                      }}
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-800">
                            {h.symptoms?.join(", ") || "Unknown symptoms"}
                          </p>
                          {h.results?.length > 0 && (
                            <p className="text-xs text-green-600 mt-1">
                              Top remedy: {h.results[0]?.remedy?.name || "—"}
                            </p>
                          )}
                        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50">
      ...
      <style>{`
        ...
        .goog-te-menu-value span { color: #15803d !important; }
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <>
                ...
                {/* Profile button — now opens profile page */}
                <button
                  onClick={() => setShowProfile(true)}
                  className="flex items-center gap-1 bg-green-50 hover:bg-green-100 border border-green-200 px-3 py-1 rounded-full transition"
                  title="View Profile"
                >
